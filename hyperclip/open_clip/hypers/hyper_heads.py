...
class CustomLinearStack(nn.Module):
    def __init__(self, input_dim, output_dims):
        super(CustomLinearStack, self).__init__()
        self.layers = nn.ModuleList()  # This will hold all the linear layers
        current_input_dim = input_dim

        # Create the layers based on the provided output dimensions
        for output_dim in output_dims:
            self.layers.append(nn.Linear(current_input_dim, output_dim))

            nn.init.normal_(self.layers[-1].weight)
            self.layers[-1].weight.data.mul_(1e-2)
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            x = nn.functional.softmax(layer(x) / np.sqrt(x.shape[1]), dim=0)
            outputs.append(x.mean(0))

        combined_output = torch.concatenate(outputs)
...
class HyperNetTr(nn.Module):
    ...
    def __init__(self, mainnet, embed_dim, vision_cfg, hyp_cfg, cast_dtype, init_enc_bias = 1.0, ws=False) -> None:
        ...
        output_dimensions = [
            32, 32, 32, 32, 16, 16, 96, 96, 96, 96, 24, 24, 144, 144, 144, 144,
            24, 24, 144, 144, 144, 144, 40, 40, 240, 240, 240, 240, 40, 40, 240,
            240, 240, 240, 80, 80, 480, 480, 480, 480, 80, 80, 480, 480, 480, 480,
            80, 80, 480, 480, 480, 480, 112, 112, 672, 672, 672, 672, 112, 112, 672,
            672, 672, 672, 112, 112, 672, 672, 672, 672, 192, 192, 1152, 1152, 1152,
            1152, 192, 192, 1152, 1152, 1152, 1152, 192, 192, 1152, 1152, 1152, 1152,
            192, 192, 1152, 1152, 1152, 1152, 320, 320, 1280, 1280
        ]
        self.linearstack = CustomLinearStack(d_model, output_dimensions)
    ...
    def forward(self, text_features):
        ...
        params_out = self.linearstack(out).float()
