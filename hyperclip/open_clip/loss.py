...
class ClipLoss(nn.Module):
    ...
    def forward(self, image_features, text_features, logit_scale, output_dict=False, model=None):

        # grad = torch.autograd.grad(image_features.mean(), model.mainnet.trunk.blocks[4][0].conv_pwl.loru, retain_graph=True)
        # print(grad)
...
def neighbour_exchange(from_rank, to_rank, tensor, group=None):
    ...
    send_op = torch.distributed.P2POp(torch.distributed.isend,
                                      tensor, to_rank, group=group)
    recv_op = torch.distributed.P2POp(torch.distributed.irecv,
                                      tensor_recv, from_rank, group=group)
...
class SigLipLoss(nn.Module):
    ...
    def _loss(self, image_features, text_features, logit_scale, logit_bias=None, negative_only=False):
        ...
        # loss = F.gelu(labels * logits).sum() / image_features.shape[0]

        return loss

    def forward(self, image_features, text_features, logit_scale, logit_bias, output_dict=False, model=None):
        # print(image_features.shape)
        # print(text_features.shape)
        # print(logit_scale.shape)
        # print(logit_bias.shape)
